# Adapted from py-vobject (https://github.com/py-vobject/vobject),
# commit 8ebd737, Apache License 2.0 (see LICENSES/py-vobject-LICENSE).
# Serves as this package's py-vobject compatibility layer; parsing
# internals are progressively delegated to the Rust core (vobject._core).
...
def foldOneLine(outbuf, input_, lineLength=75):
    ...
    The standard 75-octet case is delegated to the Rust core
    (vobject._core.fold_line), which implements identical semantics; the
    pure-Python path remains for nonstandard fold widths (e.g. the PHOTO
